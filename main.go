	...
	cmdCreate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for patchFile, patchBytes := range clusterTemplatePatches {
				...
				generateKustomizationFileCmd := exec.Command("clusterctl", "generate", "yaml")
				generateKustomizationFileCmd.Stdin = bytes.NewReader(patchBytes)
				generateKustomizationFileCmd.Stdout = kustomizationFile
				if err := runCommand(generateKustomizationFileCmd); err != nil {
					return fmt.Errorf("clusterctl generate kustomization file for %s: %s", patchFile, err)
				}
				kustomizationFile.Close()
